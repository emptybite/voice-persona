...
class JsonStore:
    ...
    def _session_title_preview(self, history: list[Any]) -> tuple[str, str]:
        ...
        source_title = first_user or first_message
        if source_title:
            title = self._truncate(source_title, 36)

        if last_message:
            role, content = last_message
            prefix = "You: " if role == "user" else ""
            preview = self._truncate(f"{prefix}{content}", 58)
    ...
    def delete_profile(self, profile_id: str) -> bool:
        with self._lock:
            ...
            removed_profiles = [p for p in profiles if p.get("id") == profile_id]
            ...
            for profile in removed_profiles:
                ...
                if avatar_url.startswith("/media/avatars/"):
                    avatar_path = self.avatars_dir / Path(avatar_url).name
